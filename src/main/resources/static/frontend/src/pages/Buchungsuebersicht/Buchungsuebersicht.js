...
import './Buchungsuebersicht.css';
import './modal.css';
import Header from '../../components/Header/HeaderLoggedIn';
import { Link, useNavigate } from 'react-router-dom';
import axios from 'axios';
import { fetchUserByToken, fetchUserById } from '../../services/UserService';

const Buchungsuebersicht = () => {
  ...
  useEffect(() => {
    const fetchBookingsAndRoomsAndUsers = async () => {
      try {
        setLoading(true);
        setError('');
        ...
        const buchungenResponse = await axios.get(`http://localhost:8080/api/buchungen/all`);
        ...
        // Nutzerdaten abrufen
        const uniqueUserIds = [...new Set(userBookings.map(buchung => buchung.gebuchtVonId))];
  ...
  if (loading) {
    return (
      <div className="Buchungsuebersicht-container">
        <Header />
        <div className="Buchungsuebersicht_content">
            ...
            <table>
              ...
              <tbody>
                {bookings.map(booking => {
                  const gebuchtVon = userNames[booking.gebuchtVonId] || 'Unbekannter Benutzer';
...
export default Buchungsuebersicht;